...
- **DRAM Module 더미 규칙 확장**: 기존 "PID 끝 `00`" 더미에 더해, **`PartClass.Module` 한정**으로 파트번호 마지막 2글자가 `B0`/`R0`인 것도 더미로 걸러 작업을 진행하지 않는다. (`PartClassifier.IsDummy`, 엑셀 import·BIN 스킵 경로 공용)
  - Comp/Comp_MDL/SSD의 끝 `B0`/`R0`는 **정상 파트**라 영향 없음(SSD는 B0/R0가 BIN 행 매핑).
