...
- 로그인 실패 의심 팝업은 로그인 화면이 실제로 남아 있을 때만 실패 처리되어야 한다. 로그인 화면이 없으면 안내 팝업으로 보고 무시한다.
...
6. 정지 버튼: 실행 버튼을 더블클릭해도 실행 직후 정지되지 않고, 1.5초 이후 `정지` 요청은 안전 지점에서 중단되어 결과 파일이 남는지 확인.
